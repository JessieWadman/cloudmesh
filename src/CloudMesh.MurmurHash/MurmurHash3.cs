namespace CloudMesh.Utils
{
    ...
    public class MurmurHash3
    {
        ...
        public static (ulong h1, ulong h2) ComputeHash(ReadOnlyMemory<byte> buffer)
        {
        ...
        public static unsafe (ulong h1, ulong h2) ComputeHash(ReadOnlyMemory<byte> buffer, int offset, int count)
        {
            if (buffer.IsEmpty)
                return (0, 0);
            if (offset < 0)
            ...
            {
                var bufferPinPointer = (byte*)bufferPin.Pointer;
                var pinPointer = bufferPinPointer + offset;
                var body = (ulong*)pinPointer;
                ...
                var tail = pinPointer + numberOfBlocks * 16;
                switch (count & 15)
                {
                    case 15:
                        k2 ^= (ulong)tail[14] << 48;

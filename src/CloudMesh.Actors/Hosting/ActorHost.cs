...
namespace CloudMesh.Actors.Hosting
{
    ...
    public class ActorHost : IRoutingTableUpdate, IActorHost, IActorLifetimeNotifications, IAsyncDisposable, IDisposable
    {
        ...
        {
            var logger = loggerFactory.CreateLogger<ActorHost>();
            try
            {
                while (!stoppingToken.IsCancellationRequested)
                {
                    await Task.Delay(5000, stoppingToken); // will throw TaskCancelledException if stoppingToken is cancelled
        ...
        public void RoutingTableUpdated(ImmutableArray<RoutingTableEntry> routingTable)
        {
